...
export function RegisterForm({ mostrar }: RegisterFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) next.email = 'Formato de email inválido'
    if (!telefono) next.telefono = 'El teléfono es requerido'
    else if (!/^\d{8,15}$/.test(telefono)) next.telefono = 'Solo dígitos, 8 a 15 caracteres'
    ...
    else if (password !== passwordConfirm) next.passwordConfirm = 'Las contraseñas no coinciden'
    if (Object.values(next).some(Boolean)) { setErrs(next); return }

    setCargando(true)
    try {
      const s = await AuthService.registro({ nombre, apellido, email, telefono, password })
      login(s)
  ...
  return (
    <form className="flex flex-col gap-1" onSubmit={handleSubmit} noValidate>
      <div className="grid grid-cols-2 gap-4">
        <div className="mb-5">
          <label htmlFor="reg-nombre" className="block mb-1.5 text-[13px] font-semibold text-slate-800 tracking-[0.01em]">Nombre <span className="text-red-500">*</span></label>
          <input id="reg-nombre" type="text" className={inputCls(errs.nombre)} value={form.nombre} onChange={(e) => set('nombre', e.target.value)} />
          <FieldError msg={errs.nombre} />
        </div>
        <div className="mb-5">
          <label htmlFor="reg-apellido" className="block mb-1.5 text-[13px] font-semibold text-slate-800 tracking-[0.01em]">Apellido <span className="text-red-500">*</span></label>
          <input id="reg-apellido" type="text" className={inputCls(errs.apellido)} value={form.apellido} onChange={(e) => set('apellido', e.target.value)} />
          <FieldError msg={errs.apellido} />
        </div>
      </div>
      <div className="mb-5">
        <label htmlFor="reg-email" className="block mb-1.5 text-[13px] font-semibold text-slate-800 tracking-[0.01em]">Email <span className="text-red-500">*</span></label>
        <input id="reg-email" type="email" className={inputCls(errs.email)} placeholder="tu@email.com" value={form.email} onChange={(e) => set('email', e.target.value)} />
        <FieldError msg={errs.email} />
      </div>
      <div className="mb-5">
        <label htmlFor="reg-telefono" className="block mb-1.5 text-[13px] font-semibold text-slate-800 tracking-[0.01em]">Teléfono <span className="text-red-500">*</span></label>
        <input id="reg-telefono" type="tel" className={inputCls(errs.telefono)} placeholder="1123456789" value={form.telefono} onChange={(e) => set('telefono', e.target.value)} />
      ...
      <button
        ...
        disabled={cargando}
      >
        {cargando ? 'Creando cuenta…' : <> Crear Cuenta <LuArrowRight size={16} /></>}
      </button>
    </form>
  )
}
